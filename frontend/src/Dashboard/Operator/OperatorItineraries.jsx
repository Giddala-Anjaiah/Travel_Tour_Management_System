...
const OperatorItineraries = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:5000/api/operator/itineraries/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingId
        ? `http://localhost:5000/api/operator/itineraries/${editingId}`
        : 'http://localhost:5000/api/operator/itineraries'
