...
const OperatorPricingAvailability = () => {
  ...
  const handlePricingSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingPricingId
        ? `http://localhost:5000/api/operator/pricing/${editingPricingId}`
        : 'http://localhost:5000/api/operator/pricing'
  ...
  const handleAvailabilitySubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/operator/availability', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteAvailability = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:5000/api/operator/availability/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
